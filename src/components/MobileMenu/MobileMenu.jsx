...
const MENU_ITEMS = [
  ...
  { label: 'Categories', submenu: ['Lifestyle', 'Style', 'Events', 'Travel', 'Music'] },
  { label: 'Shop', submenu: ['Shop Page', 'Cart', 'Checkout'] },
  { label: 'Buy Now', submenu: null },
];
                ...
                <li key={item.label} className="mobile-menu__item">
                  {item.submenu ? (
                    ...
                    >
                      ...
                      <svg
                        className={`mobile-menu__chevron${isOpen ? ' mobile-menu__chevron--open' : ''}`}
                        width="10"
                        height="6"
                        viewBox="0 0 10 6"
                        fill="none"
                  ...
                  {item.submenu && isOpen && (
                    <ul className="mobile-menu__submenu">
                      {item.submenu.map((sub) => (
                        <li key={sub} className="mobile-menu__submenu-item">
                          <a href="#" className="mobile-menu__submenu-link">{sub}</a>
                        </li>
                      ))}
                    </ul>
                  )}
